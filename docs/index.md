...
1. Gradients are not used: any specifications relating to gradient calculations
   in `ropt` are ignored.
2. The `convergence_tolerance` field of the `backend` section is ignored.
...
5. Some additional `Nomad` options can be passed as a list of strings via the
   `options` field of the `backend` section. Refer to the documentation of
   the [`NomadBackend`][ropt_nomad.nomad.NomadBackend] class for supported
   options.
